...
def update_prob(frequencies):
    p = {}
    for i in range(127):
        context = []
        if i in frequencies[0]:
            prob = frequencies[0][i]/len(frequencies[0])
            for context_num in range(1, kappa):
                context.append(i)
                if tuple(context) in frequencies[context_num]:
                    if i in frequencies[context_num][tuple(context)]:
                        prob *= frequencies[context_num][i]/len(frequencies[context_num])
                    else:
                        prob /= len(frequencies[context_num][tuple(context)])
                else:
                    break

        else:
            prob = 1/(len(frequencies[0]))
